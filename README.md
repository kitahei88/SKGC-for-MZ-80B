...
AVRはMicrochip社のATmega328PもしくはATmega88Vを使用しています。
...
ジャンパーは切り換えるスイッチ状態を出力をするのか、切り換わった状態（IPLが終わってAllRAMモードに変更された後の状態）を出力するのかを設定するためにあります。

これを利用すると例えばyanatakaさんの作成された[MZ-2000_SD](https://github.com/yanataka60/MZ-2000_SD)がMZ-80BでもMZ-2000でもROMを切り換えるスイッチに入力させることで、自動で切り換えて使えます。
詳細はDocフォルダ内のModeOut.md内に記載しています。
なんのことかわかんない場合は放置でおｋ
