...
またドキュメントに記載されている内容は開発段階のものを含み、実際の動作などは改変により異なることがあります。
...
## 動作環境 
動作するMZ-80Bが必要です。
CMTやディスプレイはMZ-80B本体のものを使用します。
外部からCMTのエミュレーションを行うような実装にしていたり、ディスプレイを外部の液晶などへ表示するようにしていた場合の動作は保証外です。
...
AVRはMicrochip社のATmega328PVを使用しています。

~~Microchip社の[Microchip Studio](https://www.microchip.com/en-us/tools-resources/develop/microchip-studio)にて開発しています。~~
[VScode](https://code.visualstudio.com/)上での [PlatformIO](https://platformio.org/)を利用して作成しました。環境は様々に選択出来ますので、ソースとしてmain.cを置いておきます。
ご自身の環境でコンパイル、デバッグなどが出来ます。
また書き込むだけであれば、バイナリとしてfirmware.hexファイルもアップしていますので、そちらを使用してください。FUSEビットに関しては[FUSE.md](./8255_ATmegaX8/FUSE.md)に記載があります。
...
**GRAMにあるジャンパ、2ピンのコネクターはなんですか？**

PL2と書かれたジャンパはMZ-2000でのモード時に、IPLはMZ-80Bのもので起動させるか、IPLからMZ-2000のもので起動させるかを変更するためのものです。
詳細はDocフォルダ内の[IPL.md](./Doc/IPL.md)内に記載しています。

これは現在の動作モードの出力するためのコネクターです。
MZ-80Bのモードの場合はH(+5V)、MZ-2000のモードの場合はL(GND)が出力されます。詳細は回路図等参考ください。

PL3と書かれたジャンパーは切り換えるスイッチ状態を出力をするのか、切り換わった状態（IPLが終わってAllRAMモードに変更された後の状態）を出力するのかを設定するためにあります。

これらを利用すると例えばyanatakaさんの作成された[MZ-2000_SD](https://github.com/yanataka60/MZ-2000_SD)がMZ-80BでもMZ-2000でもROMを切り換えるスイッチに入力させることで、自動で切り換えて使えます。
詳細はDocフォルダ内の[ModeOut.md](./Doc/ModeOut.md)内に記載しています。
なんのことかわかんない場合は放置でおｋ
